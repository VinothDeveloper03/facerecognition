...
while 1:
	...
	for face_id, tracker in trackers.items(): # iterate over current trackers
		...
		if sampleNum > 100:
			cregnumber,crfid = fs.examvales1()
			print(crfid)
			...
			cursor = conn.cursor()
			cursor.execute("SELECT  *  FROM regtb where  Regno='" + str(ss) + "' and RFID='" + str(crfid) + "'")
			...
			if data:
				...
				cursor = conn.cursor()
				cursor.execute("select * from attentb where Date='" + str(date) + "' and Regno='" + str(ss) + "'")
				data = cursor.fetchone()
				if data is None:
					conn = mysql.connector.connect(user='root', password='', host='localhost',
												   database='1faceattendancedbrfid')
